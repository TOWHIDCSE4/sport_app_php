...
class User extends Authenticatable implements HasLocalePreference, HasMedia
{
    ...
    public function joinedAndTeamEventsByDate($from_date, $to_date)
    {
        $teamIds = $this->memberTeams()->get()->pluck('id')->toArray();
        $userId = $this->id;
        return Event::leftJoin('user_event', function ($q)  {
        ...
        ->leftJoin('event_team', function ($q)  {
            ...
            $q->where('user_event.user_id', $userId)
            ->orWhereIn('event_team.team_id', $teamIds);
        })
        ->where(function($q) use ($from_date, $to_date){
            $q->where(function ($query) use ($from_date) {
                $query->where('events.start_date_time', '<=', $from_date);
                $query->where('events.end_date_time', '>=', $from_date);
            })
            ->orWhere(function ($query) use ($to_date) {
                $query->where('events.start_date_time', '<=', $to_date);
                $query->where('events.end_date_time', '>=', $to_date);
            });
        })
