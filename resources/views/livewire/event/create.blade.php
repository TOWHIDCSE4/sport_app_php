<form wire:submit.prevent="submit" class="pt-3">
    ...
    @if ($show_sport)
        <div class="form-group {{ $errors->has('event.sport_id') ? 'invalid' : '' }}">
            <label class="form-label required" for="sport">{{ trans('cruds.event.fields.sport') }}</label>
    ...
    <div class="form-group {{ $errors->has('event.end_date_time') ? 'invalid' : '' }}">
        <label class="form-label required" for="end_date_time">{{ trans('cruds.event.fields.end_date_time') }}</label>
    ...
    @if ($show_max_team)
        <div class="form-group {{ $errors->has('event.max_team') ? 'invalid' : '' }}">
            <label class="form-label required">{{ trans('cruds.event.fields.max_team') }}</label>
            <select class="form-control" wire:model="event.max_team">
                <option value="null" disabled>{{ trans('global.pleaseSelect') }}...</option>
                @foreach ($this->listsForFields['max_team'] as $key => $value)
                    <option value="{{ $key }}">{{ $value }}</option>
                @endforeach
            </select>
            <div class="validation-message">
        ...
        @if ($showApplicationType == 'individual')
            ...
            @if ($showJoinTeam == 0)
                <div class="form-group {{ $errors->has('event.max_player_per_team') ? 'invalid' : '' }}">
                    ...
                    @endif
                    <input class="form-control" type="text" name="max_player_per_team" id="max_player_per_team"
                        wire:model="event.max_player_per_team" step="1">
                    <div class="validation-message">
        ...
        @elseif ($showApplicationType == 'team')
            ...
            @if ($showJoinTeam == 0)
                <div class="form-group {{ $errors->has('event.max_team') ? 'invalid' : '' }}">
                    ...
                    <input class="form-control" type="text" name="max_team" id="max_team"
    ...
    @if ($showChargeFee == 0)
        <div class="form-group {{ $errors->has('event.fee') ? 'invalid' : '' }}">
            <label class="form-label required" for="fee">{{ trans('cruds.event.fields.fee') }}</label>
        ...
        <div class="form-group {{ $errors->has('event.currency') ? 'invalid' : '' }}">
            <label class="form-label required">{{ trans('cruds.event.fields.currency') }}</label>
            <select class="form-control" wire:model="event.currency">
                <option value="null" disabled>{{ trans('global.pleaseSelect') }}...</option>
            ...
            <div class="validation-message">
                {{ $errors->first('event.currency') }}
            </div>
            <div class="help-block">
                {{ trans('cruds.event.fields.currency_helper') }}
            </div>
        </div>
    @endif
    @if ($show_game_privacy)
